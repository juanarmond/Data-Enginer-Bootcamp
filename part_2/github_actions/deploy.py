...
def create_stack(stack_name, template_body, **kwards):
...
def update_stack(stack_name, template_body, **kwards):
    try:
        ...
        if 'No update are to be performed' in str(e):
            logging.info(f'SKIPPING UPDATE: No update to be performed as stack {stack_name}')
            return e

    cloudformation_client.get_waiter('stack_update_complete').wait(
        StackName = stack_name,
        WaiterConfig = {'Delay': 5, 'MaxAttemps': 600}
    )
